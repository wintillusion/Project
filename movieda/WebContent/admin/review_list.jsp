...
<body>
<h1 style="text-align:center">REVIEW 관리</h1>
...
<div class="links">
	...
	<%if(nowPage<=1){ //현재페이지가 1보다 작으면 이전이 안되도록.%>
		...
		<%}else{ %>
<a href="reviewList.admin?page=<%=nowPage-1 %>&searchOption=${sO}&searchWord=${sW}">&laquo;</a> 
<%} %><%for(int a=startPage;a<=endPage;a++){
			if(a==nowPage){%>
			<%=a %><!-- 현재값은 하이퍼링크 안걸리게 -->
			<%}else{ %>
				<a href="reviewList.admin?page=<%=a %>&searchOption=${sO}&searchWord=${sW}"><%=a %></a>&nbsp;
			<%} %>
		<%} //그다음 페이지를 다시 찍어주는 것. startpage부터 endpage까지 찍어준다.%>
		<%if(nowPage>=maxPage){ %>
		&raquo;<%//마지막 페이지라면 다음이 활성화 안되게. else 활성화되서 +1이 되도록. %>
		<%}else{ %>
<a href="reviewList.admin?page=<%=nowPage+1 %>&searchOption=${sO}&searchWord=${sW}">&raquo;</a>
		<%} %>
</div>
</td>
</tr>
<tr>
				<td colspan="6" align="right">
					<form method="post" action="reviewList.admin">
...
<td>
				...
				<%} %>
				<%}else{ %><%}%><!-- 들여쓰고 화살표를 넣는 것. -->
					<a style="color:black" href="reviewDetail.admin?num=<%=reviewList.get(i).getNum() %>&page=<%=nowPage %>#begin">
