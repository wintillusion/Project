...
public class RequestViewListAction implements Action {	//게시글 볼때 리스트도 함께 띄워주기
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		int listCount=requestListService.getListCount(sO,sW);//sO, sW넘겨줘야함.
		//총 리스트 수를 받아옴
		requestList = requestListService.getRequestList(page,limit,sO,sW);	//값을 넘길때 페이지에서 몇개를 가져올지?+sW,sO
		//리스트를 받아옴
		//총페이지 수
		...
		forward.setPath("/requestBoard/request_view_list.jsp");
		return forward;
	}
