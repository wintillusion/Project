...
public class AuctionListAction implements Action{
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int listCount=auctionListService.getListCount(sO,sW);//sO, sW넘겨줘야함.
		//총 리스트 수를 받아옴
		auctionList = auctionListService.getAuctionList(page,limit,sO,sW);	//값을 넘길때 페이지에서 몇개를 가져올지?+sW,sO
		//리스트를 받아옴
		//총페이지 수
		...
		request.setAttribute("auctionList", auctionList);// 결과값을 넘겨줌2 속성으로 넘겨야함.
		request.setAttribute("sW", sW);
		request.setAttribute("sO", sO);
		ActionForward forward = new ActionForward();
		forward.setPath("/admin/auction_list.jsp");
		return forward;
	}
