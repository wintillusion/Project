...
public class ReviewListAction implements Action {
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		ReviewListService requestListService = new ReviewListService();
		int listCount=requestListService.getListCount(sO,sW);//sO, sW넘겨줘야함.
		//총 리스트 수를 받아옴
		reviewList = requestListService.getReviewList(page,limit,sO,sW);	//값을 넘길때 페이지에서 몇개를 가져올지?+sW,sO
		//리스트를 받아옴
		//총페이지 수
		...
		forward.setPath("/reviewBoard/review_list.jsp");
		return forward;
	}
